...
struct sr_rt* sr_longest_prefix_match(struct sr_instance* sr, uint32_t my_ip) {
	...
	uint32_t best_match = 0, mask;
	struct sr_rt* my_rt_walker = sr->routing_table;
	while (my_rt_walker) {
		mask = my_rt_walker->mask.s_addr;
		if ((mask & my_rt_walker->dest.s_addr) == (mask & my_ip)) {
		   	if (!result || (mask > best_match)) {
		   		result = my_rt_walker;
		   		best_match = mask;
		   	}
		}
